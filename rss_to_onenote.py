...
class OneNoteSync:
    ...
    def _load_cache(self):
        cache_path = os.path.join(os.path.dirname(__file__), TOKEN_CACHE_FILE)
        if os.path.exists(cache_path):
            try:
                with open(cache_path, "r") as f: self.token_cache.deserialize(f.read())
            except Exception as e: print(f"[缓存警告] 读取令牌缓存文件 {cache_path} 失败: {e}")
    def _save_cache(self):
        cache_path = os.path.join(os.path.dirname(__file__), TOKEN_CACHE_FILE)
        if self.token_cache.has_state_changed:
            try:
                with open(cache_path, "w") as f: f.write(self.token_cache.serialize())
                self.token_cache.has_state_changed = False
            except Exception as e: print(f"[缓存错误] 保存令牌缓存文件 {cache_path} 失败: {e}")
    def get_token(self):
        token_result = None; accounts = self.app.get_accounts()
        if accounts: token_result = self.app.acquire_token_silent(SCOPES, account=accounts[0])
        if not token_result:
            print("[认证] 缓存未命中或已过期，尝试交互式登录...")
            try: token_result = self.app.acquire_token_interactive(scopes=SCOPES)
            except Exception as e: print(f"[认证错误] 交互式获取令牌失败: {e}"); return None
        if token_result and "access_token" in token_result:
            self._save_cache(); return token_result["access_token"]
        else:
            error_desc = token_result.get("error_description", "未知错误") if token_result else "未知错误"
    ...
    def _make_api_call(self, method, url, headers=None, json_data=None, data=None):
        ...
        except requests.exceptions.SSLError as ssl_err: print(f"[API 调用 SSL 错误] {ssl_err}"); print("  >> 检查服务器 SSL/TLS 配置或更新客户端库。"); return None
        except requests.exceptions.HTTPError as http_err:
            if http_err.response.status_code == 401: print("[API 调用错误] 401 未授权。")
            elif http_err.response.status_code == 403: print("[API 调用错误] 403 禁止访问。")
            elif http_err.response.status_code == 429: print("[API 调用错误] 429 请求过于频繁。")
            elif http_err.args and isinstance(http_err.args[0], requests.exceptions.SSLError): print(f"[API 调用 SSL 错误] {http_err}"); print("  >> 检查服务器 SSL 配置或更新库。")
            else: print(f"[API 调用 HTTP 错误] {http_err.response.status_code} {http_err}")
            try: print(f"  错误详情: {http_err.response.json()}")
            except: print(f"  错误响应 (非JSON): {http_err.response.text}")
            return None
        except requests.exceptions.ConnectionError as conn_err: print(f"[API 调用连接错误] {conn_err}"); return None
        except requests.exceptions.RequestException as req_err: print(f"[API 调用请求错误] {req_err}"); return None
        except Exception as e: print(f"[API 调用未知错误] {e}"); return None
    def create_onenote_page_in_app_notebook(self, title, content_html):
        url = "https://graph.microsoft.com/v1.0/me/onenote/pages"; headers = {"Content-Type": "application/xhtml+xml"}; safe_title = html.escape(title)
        ...
        if response and response.status_code == 201:
            try: response_data = response.json(); page_link = response_data.get('links', {}).get('oneNoteWebUrl', {}).get('href')
            except Exception as e: print(f"  >> 解析 API 响应时出错: {e}")
            return True
        else: return False
...
def clean_extracted_html(html_content):
    if not html_content or not isinstance(html_content, str) or html_content.startswith("[错误："): return html_content
    ...
    try:
        ...
        for tag_or_selector in tags_to_remove:
            try: elements = soup.select(tag_or_selector) if tag_or_selector.startswith(('.', '#')) else soup.find_all(tag_or_selector); [el.decompose() for el in elements]
...
def get_full_content_from_link(url, source_name, feed_url, verify_ssl=True):
    ...
    try:
        ...
        response.raise_for_status(); final_url = response.url
        ...
        except Exception as e: print(f"    >> [内容/时间抓取] 警告：lxml 解析失败 ({e})，回退 html.parser。"); soup = BeautifulSoup(response.text, 'html.parser')
        ...
        if source_name and source_name.startswith('和讯'):
            ...
            try:
                info_div = soup.select_one('div.pages-detail-info'); found_date_str = None
                if info_div:
                    for span in info_div.find_all('span', recursive=False):
                        span_text = span.get_text(strip=True)
                        if "发布日期：" in span_text:
                            date_part = span_text.split("发布日期：")[-1].strip(); match = re.search(r'^(\d{4}-\d{2}-\d{2})', date_part);
        ...
        elif source_name and source_name.startswith('和讯'): selectors.extend(['div.art_contextBox', 'div.art_context'])
        elif is_jintiankansha_source: selectors.extend(['div.rich_media_content', 'div#js_content', 'div.wx-content']) # 微信选择器
        ...
        # print(f"    >> [内容抓取] 将尝试以下选择器序列: {selectors}")
        ...
        if article_body:
            ...
            if article_body:
                for img in article_body.find_all('img'):
                    try: # 处理单个图片
                        ...
                        if src: # 确保 src 存在
                            # --- 已修正：if not src.startswith... 块内语句分行 ---
                            if not src.startswith(('http://', 'https://', 'data:')):
                                try: # 处理 urljoin 可能的错误
                                    absolute_src = urljoin(final_url, src)
                                    img['src'] = absolute_src
                                    img_fixed_count += 1
                                except Exception as url_e:
                                    print(f"    >> [图片路径修复] 尝试合并 URL 时出错 for src='{src}': {url_e}")
                            # --- 处理 data-src ---
                                ...
                                if data_src and not data_src.startswith(('http://', 'https://', 'data:')):
                                     try: # 处理 urljoin 可能的错误
                                         ...
                                         img_fixed_count += 1
                                          ...
                                          print(f"    >> [图片路径修复] 尝试合并 data-src 时出错 for data-src='{data_src}': {url_e}")
                                else: # 如果 data_src 是绝对路径或 data URI
                                     img['src'] = data_src # 直接使用 data_src 覆盖 src
                    except Exception as img_e:
                        # 捕获处理这个 img 标签时可能出现的任何其他错误
                        print(f"    >> [图片路径修复] 处理 img 标签时出错: {img_e}")
            # --- 结束图片循环 ---
            ...
            cleaned_html = clean_extracted_html(content_before_cleaning) # 调用清理
...
# --- fetch_rss_feeds 函数 (恢复完整列表) ---
def fetch_rss_feeds():
    print("\n[开始抓取所有 RSS Feeds...]") # 更新提示
    all_entries = []; processed_urls_in_run = set()
    # --- 使用恢复后的完整列表 ---
    for feed_url in ORIGINAL_FEEDS:
        if feed_url in processed_urls_in_run: continue
        source_name = FEED_SOURCES.get(feed_url, feed_url); print(f"  处理中: {source_name} ({feed_url})"); actual_url = feed_url
        try:
            feed_data = feedparser.parse(feed_url, agent=get_user_agent()['User-Agent'], request_headers=get_user_agent())
            if hasattr(feed_data, 'href') and feed_data.href != feed_url: print(f"    >> Feed URL 重定向到: {feed_data.href}"); actual_url = feed_data.href; FEED_SOURCES.setdefault(actual_url, source_name); processed_urls_in_run.add(actual_url)
            if feed_data.bozo: print(f"    警告: 解析 Feed '{source_name}' 时问题: {feed_data.bozo_exception}")
            if not feed_data.entries: print("    警告: 未获取到任何条目。"); continue
            for entry in feed_data.entries:
                entry_id = entry.get('id', entry.get('link')) or f"{entry.get('title', '无标题')}_{entry.get('published', entry.get('updated', time.time()))}"
                published_time_rss = None; pub_parsed = entry.get('published_parsed') or entry.get('updated_parsed')
                if pub_parsed:
                    try: published_time_rss = datetime.fromtimestamp(time.mktime(pub_parsed))
                    except (ValueError, OverflowError) as e: print(f"    警告: 条目 '{entry.get('title', '')[:30]}...' RSS 时间戳无效: {pub_parsed}, Error: {e}"); published_time_rss = None
                title = entry.get('title', '无标题').strip(); link = entry.get('link', '').strip(); content_summary = ""
                if 'content' in entry and entry.content: html_content = next((c.value for c in entry.content if c.type and 'html' in c.type.lower()), None); content_summary = html_content if html_content else next((f"<p>{html.escape(c.value)}</p>" for c in entry.content if c.type and 'text' in c.type.lower()), "")
                if not content_summary and 'summary' in entry: content_summary = entry.summary
                elif not content_summary and 'description' in entry: content_summary = entry.description
                if not entry_id or not title: continue
                all_entries.append({ 'id': entry_id, 'title': title, 'link': link, 'published_time_rss': published_time_rss, 'content_summary': content_summary, 'source_name': FEED_SOURCES.get(actual_url, source_name), 'feed_url': actual_url })
        except Exception as e: print(f"  错误: 抓取 Feed '{source_name}' 时出错: {e}"); import traceback; traceback.print_exc(); continue
        finally: processed_urls_in_run.add(feed_url)
    print(f"\n[RSS抓取完成] 共找到 {len(all_entries)} 条有效记录。")
    all_entries.sort(key=lambda x: x['published_time_rss'] or datetime.min, reverse=True)
    return all_entries

# --- load_processed_items, save_processed_items 函数不变 ---
def load_processed_items(filename=PROCESSED_ITEMS_FILE):
    processed = set(); filepath = os.path.join(os.path.dirname(__file__), filename)
    if os.path.exists(filepath):
        try:
            with open(filepath, 'r', encoding='utf-8') as f: processed.update(line.strip() for line in f if line.strip())
            print(f"[状态] 已加载 {len(processed)} 个已处理条目 ID 从 {filename}")
        except Exception as e: print(f"警告：读取已处理条目文件 {filename} 失败: {e}")
    else: print(f"[状态] 未找到已处理条目文件 {filename}。")
    return processed

def save_processed_items(new_ids, filename=PROCESSED_ITEMS_FILE):
    if not new_ids: return; filepath = os.path.join(os.path.dirname(__file__), filename)
    try:
        with open(filepath, 'a', encoding='utf-8') as f: f.writelines(f"{item_id}\n" for item_id in new_ids if item_id)
        print(f"[状态] 已将 {len(new_ids)} 个新处理条目 ID 保存到 {filename}")
    except Exception as e: print(f"错误：保存已处理条目文件 {filename} 失败: {e}")
...
if __name__ == "__main__":
    ...
    if not CLIENT_ID: exit("错误：AZURE_CLIENT_ID 未能加载。")
    print("\n[环境变量验证通过]")

    processed_ids = load_processed_items()
    newly_processed_ids_in_run = set()

    all_entries = fetch_rss_feeds() # 获取所有来源的 feeds

    new_entries = []
    for entry in all_entries:
        check_id = entry['id']
        if check_id not in processed_ids: new_entries.append(entry)
    ...
    if not new_entries:
        ...
        print(f"本次计划同步最多 {len(entries_to_process)} 条新条目。")

        print("\n[初始化 OneNote 连接...]")
        onenote = OneNoteSync()
        initial_token = onenote.get_token()
        if not initial_token: exit("错误：无法获取 OneNote 访问令牌。")
        print("OneNote 连接初始化成功。")

        print("\n[开始同步新条目到 OneNote...]")
        success_count = 0; fail_count = 0; total_to_process = len(entries_to_process)
        ...
        for i, entry in enumerate(entries_to_process):
            ...
            if fetched_content and isinstance(fetched_content, str) and fetched_content.startswith("[错误："):
                 ...
                 final_body_html = clean_extracted_html(entry['content_summary']) # 清理摘要
            elif fetched_content: # fetched_content 已经是清理过的了
                 final_body_html = fetched_content
            else:
                 print(f"    >> 获取原文内容失败或无效，尝试使用 RSS 摘要。")
                 final_body_html = clean_extracted_html(entry['content_summary']) # 清理摘要
            if not final_body_html or final_body_html.startswith("[错误："): # 检查清理结果
                 print("    >> RSS 摘要内容也为空或清理失败。")
                 final_body_html = "<p><i>[无法自动提取正文，请访问原文链接查看]</i></p>"

            # --- 链接移除已在 clean_extracted_html 完成 ---
            ...
            if creation_success:
                success_count += 1; newly_processed_ids_in_run.add(current_entry_id)
                print(f"  >> 处理成功 (耗时: {time.time() - item_start_time:.2f} 秒)")
            else:
                fail_count += 1; print(f"  !! 处理失败: {entry['title'][:60]}")

            time.sleep(REQUEST_DELAY)

        print(f"\n[同步统计] 本次成功 {success_count} 条，失败 {fail_count} 条。")
        if newly_processed_ids_in_run: save_processed_items(newly_processed_ids_in_run)
