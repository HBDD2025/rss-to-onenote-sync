...
def fetch_rss_feeds():
    ...
    for feed_url in ORIGINAL_FEEDS:
        if feed_url in processed_urls_in_run:
            continue
        source_name = FEED_SOURCES.get(feed_url, feed_url)
        ...
        try:
            ...
            if hasattr(feed_data, 'href') and feed_data.href != feed_url:
                ...
                FEED_SOURCES.setdefault(actual_url, source_name)
            ...
            for entry in feed_data.entries:
                ...
                all_entries.append({
                    ...
                    'source_name': FEED_SOURCES.get(actual_url, source_name),
                    'feed_url': actual_url
                })
        except Exception as e:
    ...
    if not new_entries:
        ...
        for i, entry in enumerate(entries_to_process):
            ...
            if fetched_content and isinstance(fetched_content, str) and fetched_content.startswith("[错误："):
                ...
                final_body_html = clean_extracted_html(entry['content_summary'])
            if not final_body_html or final_body_html.startswith("[错误："):
